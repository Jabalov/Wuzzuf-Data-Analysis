...
public class DAO
{
    ...
    public @ResponseBody String structure() throws JsonProcessingException
    {
    ...
    {
        JavaRDD<String> data = dataset.select("Skills").as(Encoders.STRING()).javaRDD();
        JavaRDD<String> words = data.flatMap(skill ->
                Arrays.asList(skill
                        .toLowerCase()
                        .trim()
                        .split(",")).iterator());

        List<Map.Entry> wordCounts = words
                .countByValue()
                .entrySet()
                .stream()
